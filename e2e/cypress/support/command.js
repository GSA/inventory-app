...
Cypress.Commands.add('requiredMetadata', (title) => {
    ...
    cy.task('log', 'REQUIRED_METADATA: Filled tags');
    cy.get('select[name=owner_org]').select('test-organization');
    cy.task('log', 'REQUIRED_METADATA: Selected owner_org');
    cy.get('input[placeholder="Select publisher"]').type('top level publisher');
    cy.task('log', 'REQUIRED_METADATA: Typed in publisher field');
    cy.get('input[placeholder="Select publisher"]').type('{downarrow}{enter}');
    cy.task('log', 'REQUIRED_METADATA: Selected publisher from dropdown');
    cy.get('input[name=contact_name]').type(chance.name());
    cy.task('log', 'REQUIRED_METADATA: Filled contact_name');
    cy.get('input[name=contact_email]').type(chance.email());
    cy.task('log', 'REQUIRED_METADATA: Filled contact_email');
    cy.get('input[name=unique_id]').type(chance.string({ length: 10 }));
    cy.task('log', 'REQUIRED_METADATA: Filled unique_id');
    cy.get('select[name=public_access_level]').select('public');
    cy.task('log', 'REQUIRED_METADATA: Selected public_access_level');
    cy.get('select[name=license]').select('Other');
    cy.task('log', 'REQUIRED_METADATA: Selected license');
    cy.get('input[name=licenseOther]').type(chance.url());
    cy.task('log', 'REQUIRED_METADATA: Filled licenseOther');
    cy.get('#rights_option_1').parent('.form-group').click();
    cy.task('log', 'REQUIRED_METADATA: Clicked rights_option_1');
    cy.get('#spatial_option_2').parent('.form-group').click();
    cy.task('log', 'REQUIRED_METADATA: Clicked spatial_option_2');
    cy.get('input[name=spatial_location_desc]').type(chance.sentence({ words: 2 }));
    cy.task('log', 'REQUIRED_METADATA: Filled spatial_location_desc');
    cy.get('#temporal_option_2').parent('.form-group').click();
    cy.task('log', 'REQUIRED_METADATA: Clicked temporal_option_2');
    cy.get('input[name=temporal_start_date]').type('2010-11-11');
    cy.task('log', 'REQUIRED_METADATA: Filled temporal_start_date');
    cy.get('input[name=temporal_end_date]').type('2020-11-11');
    cy.task('log', 'REQUIRED_METADATA: Filled temporal_end_date');
