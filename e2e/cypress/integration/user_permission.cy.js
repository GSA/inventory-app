...
describe('Dataset', () => {
    ...
    it('Editor can login and edit dataset', () => {
        cy.task('log', 'USER_PERM: Starting editor login and edit test');
        cy.task('log', 'USER_PERM: Logging in as an_editor');
        cy.login('an_editor', userPassword);
        cy.task('log', 'USER_PERM: Login completed, checking for username');

        // Verify login succeeded by checking for username
        cy.get('body').then(($body) => {
            const bodyText = $body.text();
            const hasUsername = bodyText.includes('an_editor');
            cy.task('log', 'USER_PERM: Body contains username: ' + hasUsername);
            if (!hasUsername) {
                cy.task('log', 'USER_PERM: ERROR - Username not found. First 200 chars of page: ' + bodyText.substring(0, 200));
            }
        });
        ...
        cy.contains('Edit', { timeout: 10000 }).should('be.visible').then(($btn) => {
            cy.task('log', 'USER_PERM: Found Edit button, clicking...');
        }).click();

        cy.task('log', 'USER_PERM: Clicked Edit button, checking URL');
        cy.url({ timeout: 10000 }).then((url) => {
            cy.task('log', 'USER_PERM: Current URL: ' + url);
            expect(url, 'URL should contain edit-new path after clicking Edit').to.include('/dataset/edit-new/test-dataset-1');
        });

        cy.task('log', 'USER_PERM: Looking for Save and Continue button');
        cy.contains('Save and Continue', { timeout: 10000 }).should('be.visible').then(($btn) => {
            cy.task('log', 'USER_PERM: Found Save and Continue button, clicking...');
        }).click();

        cy.task('log', 'USER_PERM: Clicked Save and Continue, checking URL');
        cy.url({ timeout: 10000 }).then((url) => {
            cy.task('log', 'USER_PERM: URL after Save: ' + url);
            expect(url, 'URL should still be on edit page after save').to.include('/dataset/edit-new/test-dataset-1');
        });

        cy.task('log', 'USER_PERM: Logging out');
        cy.logout();
        cy.task('log', 'USER_PERM: Logged out successfully');
    });
