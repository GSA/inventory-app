...
describe('DCAT-US Export', () => {
    ...
    it('Save resource file to inventory', () => {
        ...
        cy.task('log', 'TEST: Filled additional metadata');
        cy.get('button[type=button]')
            .contains('Save and Continue')
            .then(($btn) => {
                cy.task('log', 'TEST: Found Save and Continue button, clicking...');
            })
            .click()
            .then(() => {
                cy.task('log', 'TEST: Clicked Save and Continue, now on resource page');
                cy.get('input[name="resource\\.resource_type"]').then(($input) => {
                    if ($input.length > 0) {
                        cy.task('log', 'TEST: Found resource_type input field');
                    } else {
                        cy.task('log', 'TEST: ERROR - resource_type input field NOT FOUND');
                    }
                });
                cy.get('input[name="resource\\.conformsTo"]').then(($input) => {
                    if ($input.length > 0) {
                        cy.task('log', 'TEST: Found OLD conformsTo input field (should not exist!)');
                    } else {
                        cy.task('log', 'TEST: conformsTo field does not exist (expected)');
                    }
                });
                cy.resourceUpload();
                cy.task('log', 'TEST: Resource upload completed');
                cy.get('button[type=button]')
                    .contains('Finish and publish')
                    .then(($btn) => {
                        cy.task('log', 'TEST: Found Finish and publish button, clicking...');
                    })
                    .click()
                    .then(() => {
                        cy.task('log', 'TEST: Clicked Finish and publish');
                        cy.get('.resource-list').find('.resource-item').should('have.length', 1);
                        cy.task('log', 'TEST: Found 1 resource in resource list');
                        ...
                        cy.request('/api/3/action/package_show?id=' + dataset_title).then((response) => {
                            cy.task('log', 'TEST: API Response Status: ' + response.status);
                            cy.task('log', 'TEST: Dataset private: ' + response.body.result.private);
                            cy.task('log', 'TEST: Number of resources: ' + response.body.result.resources.length);
                            ...
                            if (response.body.result.resources.length > 0) {
                                ...
                                cy.task('log', 'TEST: Resource keys: ' + resourceKeys);
                                cy.task('log', 'TEST: resource_type value: ' + JSON.stringify(resource.resource_type));
                                cy.task('log', 'TEST: conformsTo value: ' + JSON.stringify(resource.conformsTo));
