...
@test "resource is accessible for the world" {
  # have to login to get the location of the resource download
  login
  ...
  resource_id=$(curl --fail --location --cookie ./cookie-jar --request GET 'http://app:5000/api/3/action/package_show?id=test-dataset-2' | jq -r '.result.resources[0].id')
  package_id=$(curl --fail --location --cookie ./cookie-jar --request GET 'http://app:5000/api/3/action/package_show?id=test-dataset-2' | jq -r '.result.id')
