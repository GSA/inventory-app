          ...
          steps {
            ansiColor('xterm') {
              ...
              dir('deploy') {
                ...
                sh 'bin/jenkins-deploy ping sandbox inventory-web'
                sh 'bin/jenkins-deploy deploy sandbox inventory.yml --limit inventory-web'
              }
            }
          }
        }
      ...
      stages {
        ...
        stage('deploy:staging') {
          ...
          steps {
            ansiColor('xterm') {
              dir('deploy') {
                sh 'bin/jenkins-deploy ping staging inventory-web'
                sh 'bin/jenkins-deploy deploy staging inventory.yml --limit inventory-web'
              }
            }
          }
        }
        stage('deploy:production') {
          ...
          steps {
            ansiColor('xterm') {
              dir('deploy') {
                sh 'bin/jenkins-deploy ping production inventory-web'
                sh 'bin/jenkins-deploy deploy production inventory.yml --limit inventory-web'
              }
            }
          }
        }
